...
export function registerRenderHandlers(broadcastProgress) {
    ...
    ipcMain.handle('render-encoder', async (event, { project, hasComposite, singleRegionCode }) => {
        try {
            ...
            const compositePath = hasComposite
                ? path.join(tempRoot, 'composite')
                : path.join(tempRoot, singleRegionCode)
    ...
    ipcMain.on('progress-connect', () => {
        broadcastProgress({ type: 'connected' })
    })

    ipcMain.handle('get-video-path', async () => 'p5mv://output.mp4')
}
