...
const p5mv = {
    width: captureInput.dims[0],
    height: captureInput.dims[1],
    dims: captureInput.dims,
    get i() {
        return __frameIndex
    },
    get percentElapsed() {
        return (__frameIndex / (captureInput.frameCount - 1)) * 100;
    },
    setup: (webgl) => {
        createCanvas(...captureInput.dims, webgl ? WEBGL : undefined)
        pixelDensity(1)
        noLoop()
...
captureInput.inputs.map(input => {
    if (p5mv.hasOwnProperty(input.id)) {
        throw new Error(`Input ID conflict: ${input.id}`);
    }
...
p5.prototype.createCanvas = function (...args) {
    ...
    return _createCanvas.call(this, ...captureInput.dims, args[2]);
}

p5.prototype.pixelDensity = () => null

p5.prototype.loop = () => null

p5.prototype.frameRate = () => null


// const _setup = p5.prototype.setup

// p5.prototype.setup = function (...args) {
