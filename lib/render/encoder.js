...
export async function encodeFramesToVideo(options) {
    ...
    const {
        compositePath,  // Path to composite frames
    ...
    // Create p5mv/Renders folder in user's videos directory
    const p5mvVideosDir = path.join(videosDir, 'p5mv', 'Renders');
    const tempOutputVideoPath = path.join(p5mvVideosDir, 'output.mp4');

    try {
        // Ensure the p5mv/Renders directory exists
        await fs.ensureDir(p5mvVideosDir);

        // Check if composite directory exists and has files
        if (!await fs.pathExists(compositePath)) {
            throw new Error(`Composite directory does not exist: ${compositePath}`);
        }

        const frameFiles = (await fs.readdir(compositePath))
            .filter(file => file.endsWith('.png'))
            .sort(); // Ensure proper ordering

        console.log(`Encoding video to ${tempOutputVideoPath} from frames in ${compositePath}`);
