...
export async function captureFrames(input = {
    scene: 'blinds',
    frameCount: 20,
    dims: [1920, 1080],
    brush: false,
    outDir: path.join(__dirname, '..', '..', 'output', 'frames')
}) {
    // Coerce dims to numbers to avoid 0×0 canvases from string inputs
    const W = Number(input.dims?.[0] ?? 0);
    const H = Number(input.dims?.[1] ?? 0);
    if (!Number.isFinite(W) || !Number.isFinite(H) || W <= 0 || H <= 0) {
        throw new Error(`Invalid dims: got [${input.dims}]. Expected positive integers.`);
    }
...
<head>
  ...
  <script>const captureInput = ${JSON.stringify({ ...input, dims: [W, H] })};</script>
  <script>${fs.readFileSync(path.join(__dirname, 'captureLib.js'), 'utf8')}</script>
</head>
<body>
  <main></main>
  <script>${fs.readFileSync(path.join(projectRoot, 'scenes', input.scene + '.js'), 'utf8')}</script>
</body>
</html>`.trim();

    const outDir = input.outDir || path.join(projectRoot, 'output', 'frames');
    ...
    try {
        ...
        console.log(`Saved ${input.frameCount} alpha PNGs to: ${outDir}`);
    } finally {
        await browser.close();
    }
}
