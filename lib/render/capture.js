...
/**
 * Generate a thumbnail for a sketch
 * @param {Object} options - Thumbnail generation options
 * @param {string} options.sceneId - Scene identifier
 * @param {string} [options.scenePath] - Path to scene directory (defaults to open-project temp directory)
 * @param {string} [options.outPath] - Output path for thumbnail (defaults to temp directory)
 * @param {number} [options.width=400] - Thumbnail width
 * @param {number} [options.height=300] - Thumbnail height
 * @param {number} [options.frameIndex=0] - Which frame to capture as thumbnail
 * @param {Object} [options.inputs={}] - Input values to pass to the sketch
 * @returns {Promise<{success: boolean, path: string}>} - Success status and path to thumbnail
 */
export async function generateThumbnail(options) {
    const {
        sceneId,
        scenePath = path.join(app.getPath('temp'), 'open-project', 'sketches', options.sceneId),
        outPath = path.join(app.getPath('temp'), 'p5mv-thumbnails', options.sceneId),
        width = 400,
        height = 300,
        frameIndex = 0,
        inputs = []
    ...
    // Build minimal capture input for thumbnail
    const captureInput = {
        scene: sceneId,
        frameCount: 1,
        dims: [width, height],
    ...
    const thumbnailPath = path.join(outPath, 'f000000.png');
