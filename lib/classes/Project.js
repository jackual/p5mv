import { beatsToFrameDuration } from "../timeUtils"
...
export default class Project {
  constructor(string = "{}") {
    ...
    this.tracks = data.tracks ? data.tracks.map((track, index) => new Track(track, this, index)) : [new Track({}, this, 0)]
    this.view = {
      beatWidth: 10,
      start: 0,
      get end() {
    ...
    this.snap = 1
    this.clipboard = null
    this.openScenes = [] // Dynamically loaded scenes from project directory
    this.meta = {
    ...
    return this.tracks.map(track => {
      return track.regions
        ...
        .map(region => {
          return [
            region.code,
            beatsToFrameDuration(region.position, this.meta.bpm, this.meta.fps),
            beatsToFrameDuration(region.length, this.meta.bpm, this.meta.fps),
            region.blend.opacity,
            region.blend.mode
          ]
        })
    })
  ...
  copy() {
    this.clipboard = this.selected[0].export()
  }

  paste() {
    if (this.clipboard) {
      const copy = this.clipboard
      copy.position = this.newRegion[1]
      const region = new Region(copy, this, 0, this.tracks[this.newRegion[0]])
      this.tracks[this.newRegion[0]].regions.push(region)
      this.reindexAll()
    }
  }
