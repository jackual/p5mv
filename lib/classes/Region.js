...
export default class Region {
  constructor(object, project, index, track) {
    // Defensive check to ensure object is valid
    if (!object || typeof object !== 'object') {
      throw new Error('Region constructor requires a valid object');
    }
  ...
  initialiseInputs() {
    ...
    this.inputs = []
    const scene = this.project.openScenes?.find(scene => scene.id === this.sceneId)
    if (!scene) return
  ...
  get scene() {
    return this.project.openScenes?.find(scene => scene.id === this.sceneId) || null
  }
  ...
  export() {
    // Build inputData from current property states
    const inputData = {}
    this.inputs.forEach(property => {
      inputData[property.id] = property.export()
    })
