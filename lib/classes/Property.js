...
export default class Property {
    ...
    exportForRender(project) {
        switch (this.settings.mode) {
            case 'static':
                return { id: this.id, value: this.settings.value, mode: 'static' }
            case 'unset':
                return { id: this.id, value: this.default, mode: 'static' }
            case 'dynamic':
                return {
                    id: this.id, values: this.settings.value.map(item => {
    ...
    getTweenFromDelta(delta) {
        ...
        if (!next || !prev || this.propertyType.disableTween) {
