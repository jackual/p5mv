...
async function getExternalScripts(dom, outputPath) {
    ...
    for (const script of scripts) {
        if (script.src) {
            if (isUrlExternal(script.src)) {
                ...
                // Strip query parameters and hash from URL before extracting filename
                const urlWithoutQuery = script.src.split('?')[0].split('#')[0];
    ...
    for (const link of links) {
        if (link.rel === 'stylesheet' && isUrlExternal(link.href)) {
            await fs.ensureDir(externalPath);
            // Strip query parameters and hash from URL before extracting filename
            const urlWithoutQuery = link.href.split('?')[0].split('#')[0];
            const filename = path.basename(urlWithoutQuery);
...
async function ensureP5mvScripts(dom, pathname) {
    const scripts = dom.window.document.querySelectorAll('script')
    let hasBrowserLib = false

    for (const script of scripts) {
        if (script.src) {
            const scriptPath = path.join(pathname, script.src)
            const scriptFilename = path.basename(script.src)

            // Check if this is the browser library (p5mv.js or p5mv.min.js)
            if (scriptFilename.includes('p5mv.js') || scriptFilename.includes('p5mv.min.js')) {
                hasBrowserLib = true
                script.setAttribute('data-p5mv-type', 'p5mv-browser-lib')
            } else if (await isP5(scriptPath)) {
    ...
    // Remove browser library and replace with capture library
    if (hasBrowserLib) {
        const browserLibScripts = dom.window.document.querySelectorAll('script[data-p5mv-type="p5mv-browser-lib"]')
        for (const script of browserLibScripts) {
            script.remove()
        }
    }
...
async function generateThumbnailForScene(pathname) {
    ...
    try {
        // Read scene info to get input definitions and defaults
        let dom = await getDom(indexHtml)
        const sceneInfo = await readProjectInfo(dom) || {}
        
        // Build inputs array with default values in the format expected by captureLib
        const inputs = (sceneInfo.inputs || []).map(input => {
            const propertyType = PropertyTypes[input.type] || PropertyTypes['text']
            const convertedValue = propertyType.set ? propertyType.set(input.default) : input.default
